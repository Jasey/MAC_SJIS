...
struct MemoryPage_tag {
	int                 cell_num;				// 内存块个数
	int                 use_cell_num;		// 已使用的内存块个数
	MemoryPageList      next;			// 下一个内存页
	Cell                cell[1];					// 起始内存块只有一个
};

// 存储器
struct MEM_Storage_tag {
	MemoryPageList      page_list;		//内存页面
	int                 current_page_size;	// 页大小
};
	...
	char *filename, int line, int page_size)
{
	...
	storage = MEM_malloc_func(controller, filename, line,sizeof(struct MEM_Storage_tag));		// 开辟一个存储器空间
	storage->page_list = NULL;																									// 此时存储器中的页是空的
	assert(page_size >= 0);
	if (page_size > 0)
	{
	...
	size_t size)
{
	...
	cell_num = ((size - 1) / CELL_SIZE) + 1;								// 计算所需的内存块个数
