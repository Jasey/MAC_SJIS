...
void crb_add_std_fp(CRB_Interpreter *inter)
{
    ...
    CRB_add_global_variable(inter, "STDIN", &fp_value);		//将stdin添加到全局变量列表中

    fp_value.u.native_pointer.pointer = stdout;				
    CRB_add_global_variable(inter, "STDOUT", &fp_value);	// 将stdout添加到全局变量链表中

    fp_value.u.native_pointer.pointer = stderr;
    CRB_add_global_variable(inter, "STDERR", &fp_value);	// 将stderr添加到全局变量列表中
}
