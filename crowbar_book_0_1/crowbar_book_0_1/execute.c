...
// 执行表达式语句，表达式有很多种
static StatementResult execute_expression_statement(CRB_Interpreter *inter, LocalEnvironment *env,
                             Statement *statement)
{
    ...
    v = crb_eval_expression(inter, env, statement->u.expression_s);	// 表达式求值（statement->u.expression_s是表示语句是一个表达式，statement->u.expression_s指向真正的表达式）
    if (v.type == CRB_STRING_VALUE)
	{
        crb_release_string(v.u.string_value);											// 有疑问，这里不是应该增加的么？（）
    }
...
// elsif语句
static StatementResult execute_elsif(CRB_Interpreter *inter, LocalEnvironment *env,
...
// if句子
static StatementResult execute_if_statement(CRB_Interpreter *inter, LocalEnvironment *env,
                     Statement *statement)
{
    StatementResult result;
    CRB_Value   cond;
...
// while句子
static StatementResult execute_while_statement(CRB_Interpreter *inter, LocalEnvironment *env,
                        Statement *statement)
{
    StatementResult result;
    CRB_Value   cond;
...
// for句子
static StatementResult execute_for_statement(CRB_Interpreter *inter, LocalEnvironment *env,
                      Statement *statement)
{
    StatementResult result;
    CRB_Value   cond;
...
// return句子
static StatementResult execute_return_statement(CRB_Interpreter *inter, LocalEnvironment *env,
                         Statement *statement)
{
    StatementResult result;
